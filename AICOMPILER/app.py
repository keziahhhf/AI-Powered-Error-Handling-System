...
def clean_error(err_msg):
    # Remove file paths like "/mount/src/.../user.py"
    import re
    return re.sub(r'File ".*user\.(py|c|cpp|java|js)"', 'File "user code"', err_msg)
...
if st.button("Run Code"):
    ...
    if compiler:
        ...
        if compile_run.stderr:
            ...
            st.code(clean_error(compile_run.stderr))
    ...
    try:
        ...
        if execute.stderr:
            ...
            st.code(clean_error(execute.stderr))
            st.subheader("AI Explanation")
